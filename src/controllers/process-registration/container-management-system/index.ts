...
const fetchContainerManagementPagination = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (req.query.pagination === "true") {
            const { count, rows } = await ContainerManagementSystem.findAndCountAll({
            ...
            const result = await ContainerManagementSystem.findAll({
                where: whereCondition,
                include: [
                    {
                        model: Country, as: 'country'
...
const fetchContainerManagement = async (req: Request, res: Response) => {
    try {
        const result = await ContainerManagementSystem.findOne({
            where: {
                id: req.query.id
            },
            include: [
        ...
        let userData = [];
        if (result) {
            for await (let user of result.containerManagmentUser_id) {
...
const deleteContainerManagementSystem = async (req: Request, res: Response) => {
    try {
        const knit = await ContainerManagementSystem.findOne({
            where: {
                id: req.body.id
            },
        });
        ...
        const user = await User.findOne({
            where: {
                id: knit.containerManagmentUser_id
            },
        });
        ...
        const containerManagement = await ContainerManagementSystem.destroy({
            where: {
                id: req.body.id
            }
        });
        res.sendSuccess(res, { containerManagement });
    } catch (error: any) {
        console.log(error);
        return res.sendError(res, error.message);
    }
...
const checkContainerManagementSystem = async (req: Request, res: Response) => {
    try {
        let whereCondition = {};
        if (req.body.id) {
            whereCondition = {
...
export {
    fetchContainerManagementPagination,
    fetchContainerManagement,
    deleteContainerManagementSystem,
    checkContainerManagementSystem
};  