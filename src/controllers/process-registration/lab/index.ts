...
const fetchLabPagination = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (req.query.pagination === "true") {
            let data: any = [];
            const { count, rows } = await Lab.findAndCountAll({
...
const fetchLab = async (req: Request, res: Response) => {
    try {
        ...
        if (result) {
            for await (let user of result.LabUser_id) {
...
const deleteLab = async (req: Request, res: Response) => {
    try {
        const partner = await Lab.findOne({
            where: {
                id: req.body.id
            },
        });
        ...
        const lab = await Lab.destroy({
            where: {
                id: req.body.id
            }
        });
        res.sendSuccess(res, { lab });
    } catch (error: any) {
        console.log(error);
        return res.sendError(res, error.message);
      }
...
const checkLab = async (req: Request, res: Response) => {
    try {
        let whereCondition = {};
        if (req.body.id) {
            whereCondition = {
