...
const createMandi = async (req: Request, res: Response) => {
    try {
        let userIds = [];
        ...
        const result = await Mandi.create(data);
...
const fetchMandiPagination = async (req: Request, res: Response) => {
    const searchTerm = req.query.search || '';
    const sortOrder = req.query.sort || 'asc';
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 10;
    const programId: any = req.query.programId as string;
    ...
    try {
        ...
        if (req.query.pagination === "true") {
            const { count, rows } = await Mandi.findAndCountAll({
            ...
            const result = await Mandi.findAll({
                where: whereCondition,
                include: [
                    {
                        model: Country, as: 'country'
...
const fetchMandi = async (req: Request, res: Response) => {
    try {
        ...
        let brands;
        if (result) {
            for await (let user of result.mandiUser_id) {
...
const updateMandi = async (req: Request, res: Response) => {
    try {
        let userIds = [];
        for await (let user of req.body.userData) {
            const userData = {
        ...
        const result = await Mandi.update(data, { where: { id: req.body.id } });
        res.sendSuccess(res, result);
    } catch (error: any) {
        console.log(error);
        return res.sendError(res, error.message);
...
const deleteMandi = async (req: Request, res: Response) => {
    try {
        ...
        const mandi_user = await Mandi.destroy({
            where: {
                id: req.body.id
            }
        });
        res.sendSuccess(res, { mandi_user });
    } catch (error: any) {
        return res.sendError(res, error.message);
    }
}
...
const checkMandi = async (req: Request, res: Response) => {
    try {
        let whereCondition = {};
        if (req.body.id) {
            whereCondition = {
