...
const createTraining = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (req.body.processor === 'Mandi') {
            if (req.body.processorName) {
                let create = await ProcessTrainingProcessStatus.create({
                    mandi_id: Number(req.body.processorName),
                    process_training_id: training.id,
                    status: 'Pending'
                })
            } else {
                let data = await Mandi.findAll({ where: { country_id: req.body.countryId, state_id: req.body.stateId } });
        ...
        if (req.body.processor === 'Mill') {
            if (req.body.processorName) {
                let create = await ProcessTrainingProcessStatus.create({
                    mill_id: Number(req.body.processorName),
                    process_training_id: training.id,
                    status: 'Pending'
                })
            } else {
                let data = await Mill.findAll({ where: { country_id: req.body.countryId, state_id: req.body.stateId } });
                for await (let obj of data) {
                    let create = await ProcessTrainingProcessStatus.create({
                        mill_id: obj.id,
                        process_training_id: training.id,
    ...
    try {
        ...
        if (stateId) {
            const idArray: number[] = stateId
                .split(",")
                .map((id: any) => parseInt(id, 10));
            whereCondition.state_id = { [Op.in]: idArray.map(id => Sequelize.literal(`CAST(${id} AS INTEGER)`)) };
        }        
        if (stateId) {
            const idArray: number[] = stateId
                .split(",")
...
const fecthTrainingStatusSpecific = async (req: Request, res: Response) => {
    try {
        ...
        const whereCondition: any = {};
        const { millId, mandiId, weaverId, knitterId, traderId, garmentId, mode, date }: any = req.query;
        if (millId) {
    ...
    try {
        ...
        for await (const [index, item] of training.entries()) {
            const rowValues = Object.values({
                ...
                processor: item.mill ? item.mill.name :
                    item.mandi ? item.mandi.name :'',
