...
import Spinner from "../../models/mills.model";
import Ginner from "../../models/mandi.model";
...
const createQualityParameter = async (req: Request, res: Response) => {
    try {
        ...
        for await (const quality of req.body.testReport) {
            const data = {
                mandi_id: quality.mandiId ? quality.mandiId : undefined,
                process_id: quality.processId,
                mill_id: quality.millId ? quality.millId : undefined,
...
const fetchQualityParameterPagination = async (req: Request, res: Response) => {
    ...
    const { millId, mandiId, brandId, countryId, stateId, date, type }: any = req.query
    ...
    try {
        ...
        if (type) {
            if(type === 'mandi'){
                whereCondition.mandi_id = { [Op.not]: null }
            }
            if(type === 'mill'){
                whereCondition.mill_id = { [Op.not]: null }
            }
        }
        ...
        if (brandId) {
            const idArray: number[] = brandId
                .split(",")
                .map((id: any) => parseInt(id, 10));
            whereCondition[Op.or] = [{ ["$mandi.brand$"]: { [Op.overlap]: idArray } },
            { ["$mill.brand$"]: { [Op.overlap]: idArray } }];
        }
        ...
        if (countryId) {
            const idArray: number[] = countryId
                .split(",")
                .map((id: any) => parseInt(id, 10));
            whereCondition[Op.or] = [{ ["$mandi.country_id$"]: { [Op.in]: idArray } },
            { ["$mill.country_id$"]: { [Op.in]: idArray } }];
        }
        if (stateId) {
            const idArray: number[] = stateId
                .split(",")
                .map((id: any) => parseInt(id, 10));
            whereCondition[Op.or] = [{ ["$mandi.state_id$"]: { [Op.in]: idArray } },
            { ["$mill.state_id$"]: { [Op.in]: idArray } }];
        }
        if (date) {
        ...
        let include = [
            {
                model: MandiProcess, as: 'process'
            },
            {
                model: Mandi, as: 'mandi'
            },
            {
                model: Mill, as: 'mill'
            },
        ...
        if(searchTerm){
            data = rows?.filter((row: any) => {
                ...
                (row.mill && row.mill.name && row.mill?.name.toLowerCase().includes(searchTerm.toLowerCase())) || 
                (row.mandi && row.mandi.name && row.mandi?.name.toLowerCase().includes(searchTerm.toLowerCase())) || 
