...
const createTransaction = async (req: Request, res: Response) => {
  try {
    ...
    const data: any = {
      ...
      mapped_mandi: req.body.mandi,
      vehicle: req.body.vehicle,
      payment_method: req.body.paymentMethod,
      proof: req.body.proof,
      riceVariety_id: req.body.riceVarietyId,
      status: "Pending",
    ...
    if (req.body.farmId) {
      ...
      data.estimated_paddy = Number(farm.total_estimated_paddy);
      data.available_paddy = Number(farm.total_estimated_paddy) - Number(farm.paddy_transacted || 0);
    }
 
    const transaction = await Transaction.create(data);
    const s = await Farm.update({
      paddy_transacted: Number(farm.paddy_transacted || 0) + Number(req.body.qtyPurchased)
...
const fetchTransactions = async (req: Request, res: Response) => {
  ...
  const RiceVarietyId: string = req.query.RiceVarietyId as string;
  ...
  try {
    ...
    if (searchTerm) {
      whereCondition[Op.or] = [
        ...
        { "$rice_variety.variety_name$": { [Op.iLike]: `%${searchTerm}%` } },
      ];
    }
  ...
  try {
    ...
    const data = await Transaction.findAll({
      attributes: [
        ...
        [Sequelize.col('"farm"."total_estimated_paddy"'), 'total_estimated_paddy'],
        'available_paddy',
        'qty_purchased',
        [
          sequelize.literal('("available_paddy" - CAST("qty_purchased" AS DOUBLE PRECISION))'),
          'qty_stock'
        ]
      ],
      where: {
...
const updateTransaction = async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.farmId) {
      ...
      data.estimated_paddy = farm.total_estimated_paddy;
      data.available_paddy = farm.total_estimated_paddy - (farm.paddy_transacted || 0);
    }
    ...
    let s = await Farm.update({
      paddy_transacted: (farm.paddy_transacted || 0) + Number(req.body.qtyPurchased)
    }, { where: { id: req.body.farmId } });
    res.sendSuccess(res, transaction);
  } catch (error: any) {
    console.log(error)
...
const allVillageCottonData = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { count, rows } = await Farm.findAndCountAll({
      attributes: [
        [sequelize.col('"farmer"."id"'), 'farmer_id'],
        [sequelize.literal(`CASE WHEN "farmer"."lastName" <> '' THEN CONCAT("farmer"."firstName", ' ', "farmer"."lastName") ELSE "farmer"."firstName" END`),'farmer_name',],
        [sequelize.fn('COALESCE', sequelize.fn('SUM', Sequelize.literal('CAST("farms"."total_estimated_paddy"AS DOUBLE PRECISION)')), 0), 'estimated_qty'],
        [sequelize.fn('COALESCE', sequelize.fn('SUM', Sequelize.literal('CAST("farms"."paddy_transacted" AS DOUBLE PRECISION)')), 0), 'sold_qty'],
        [sequelize.literal('(COALESCE(SUM(CAST("farms"."total_estimated_paddy" AS DOUBLE PRECISION)), 0) - COALESCE(SUM(CAST("farms"."paddy_transacted" AS DOUBLE PRECISION)), 0))'), 'available_qty'],
      ],
      include: [
        {
          model: Farmer,
          as: 'farmer',
          attributes: [], // Remove this line if you want to include farmer attributes
        }
      ],
      where: whereCondition,
      group: ['farmer.id'], // Group by farmer id
      order: [
        [
          'farmer_id', 'desc' // Change to 'farmer_id'
        ]
      ],
      offset: offset,
      limit: limit,
  ...
  try {
    if (req.query.villageId) {
      ...
      const estimatedQty = await Farm.findOne({
        attributes: [
          [Sequelize.fn('SUM', Sequelize.col('farms.total_estimated_paddy')), 'estimated_qty']
        ],
...
const deleteTransaction = async (req: Request, res: Response) => {
  try {
    ...
    if (trans.dataValues.farm_id) {
      ...
      let s = await Farm.update({
        paddy_transacted: (Number(farm.paddy_transacted) || 0) + (Number(trans.dataValues.qty_purchased) || 0)
                                ...
                                if (!farmer) {
                                  ...
                                  if (!grade) {
                                    ...
                                    if (!farm) {
                                      ...
                                      let available_cotton = (Number(farm.total_estimated_paddy) || 0) - (Number(farm.paddy_transacted) || 0);
        ...
        if (
          ...
          if (transactionExist) {
            ...
            let transactionData: any = {
              ...
              mapped_mandi: mandi.id,
              vehicle: data.vehicle ? data.vehicle : "",
              payment_method: data.paymentMethod ? data.paymentMethod : "",
              proof: data.proof ? data.proof : "",
              status: 'Pending',
            };
            let available_cotton = Number(farm.total_estimated_paddy) - Number(farm.paddy_transacted);
            transactionData.estimated_paddy = Number(farm.total_estimated_paddy);
            transactionData.available_paddy = available_cotton;
            ...
            let s = await Farm.update({
              paddy_transacted: (Number(farm.paddy_transacted) || 0) + (Number(transactionData.qty_purchased) || 0)
            }, { where: { id: farm.id } });
            pass.push({
              success: true,
              data: result,
...
const exportProcurement = async (req: Request, res: Response) => {
  ...
  try {
    const searchTerm = req.query.search || "";
    let whereCondition: any = {};
    const { status, countryId, stateId, brandId, farmGroupId, seasonId, programId, mandiId, startDate, endDate }: any = req.query;
    if (countryId) {
      const idArray: number[] = countryId
        .split(",")
        .map((id: any) => parseInt(id, 10));
    ...
    const transaction = await Transaction.findAll({
      attributes: [
        ...
        [Sequelize.col('"mandi"."name"'), 'mandies'],
        [Sequelize.col('"rice_variety"."variety_name"'), 'rice_varieties']
      ],
      where: whereCondition,
      include: [
        {
...
//Export the export details through excel file
const exportMandiProcurement = async (req: Request, res: Response) => {
  const excelFilePath = path.join("./upload", "Ginner_transactions.xlsx");
  const { farmerId,seasonId, RiceVarietyId, programId, mandiId, villageId }: any = req.query;
  const whereCondition: any = {};
  const searchTerm = req.query.search || "";
  try {
    if (!mandiId) {
      return res.sendError(res, 'PLEASE_SEND_MANDI_ID')
    }
    ...
    const transaction = await Transaction.findAll({
      attributes: [
        ...
        [Sequelize.col('"farmer"."code"'), 'farmerCode'],
        [Sequelize.fn("concat", Sequelize.col('"farmer"."firstName"'), Sequelize.col('"farmer"."lastName"')), "farmerName"],
        [Sequelize.col('"rice_variety"."variety_name"'), 'riceVariety'],
