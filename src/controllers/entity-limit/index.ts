...
const fetchEntityLimits = async (req: Request, res: Response) => {
    ...
    try {
        ...
        for await (let [index, item] of rows.entries()) {
            let [millCount, mandiCount] = await Promise.all([
                Mill.count({ where: { brand: { [Op.contains]: [item.id] } } }),
                ...
                Mandi.count({ where: { brand: { [Op.contains]: [item.id] } } }),
                // Garment.count({ where: { brand: { [Op.contains]: [item.id] } } })
            ]);
            let used_limit = (millCount + mandiCount) || 0;
            result.push({
                id: item.id,
